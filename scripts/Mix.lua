...
function selectMaterials()
	...
	while(n<anum)do
		local labels =Label:new(30,30+n*10,20,15,tpt.element(materials[n][1]));
		w:addComponent(labels);
		n=n+1;
	end
	n=0;
	
	local label=Label:new(100,10,20,15,"Percentage")
	w:addComponent(label);
	
	while(n<anum)do
		local labels =Label:new(100,30+n*10,20,15,materials[n][0]*100);
		w:addComponent(labels);
		n=n+1;
	end
	n=0;
	
	textboxp= Textbox:new(260,30,50,15,"","Perc.");
	w:addComponent(textboxp);
	...
	buttonadd:action(function()
		materials=matrAdd(textboxp:text()/100,textboxm:text(),materials);
...
function matr()
	...
	materials[0][0]=0.1;
	materials[0][1]=tpt.element("WOOD");
	materials[1][0]=0.1;
	materials[1][1]=tpt.element("PLNT");
	--array[4][0]=0.01;
...
function matrAdd(p,str,array)
	array[anum][0]=p;
	array[anum][1]=tpt.element(str);
	anum=anum+1;
	return(array);	
end
...
function update(i,x,y,ss,nt)
	...
	while(m<anum)do
			...
			if(ran<materials[m][0]+o)then
				...
				sim.partCreate(-2,x,y,materials[m][1]);
...
local mixm = elements.allocate("elem", "mix")
elements.element(mixm, elements.element(elements.DEFAULT_PT_DMND))
elements.property(mixm, "Name", "MIX")
elements.property(mixm, "Description", "Compound material")
